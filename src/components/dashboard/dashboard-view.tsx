...
function StatCard({ label, value, delta, icon, color }: {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
      ...
      {delta && <div className="text-xs text-emerald-500">{delta} <span className="text-gray-400 font-normal">vs last week</span></div>}
    </div>
  );
}
...
function DocCard({ doc, onClick, idx }: { doc: import("@/types").Doc; onClick: () => void; idx: number }) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 cursor-pointer hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-md transition-all"
    >
      <div className="flex items-start gap-2.5 mb-3">
        <div className="w-9 h-9 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 flex items-center justify-center text-lg flex-shrink-0">
          {doc.emoji}
        </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.03 }}
      ...
      <div className="relative flex-shrink-0">
        ...
        <span className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 flex items-center justify-center">
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-6xl mx-auto px-12 py-10">
          {/* Header */}
          <div className="mb-8">
            <div className="text-sm text-gray-400 mb-1.5">{formatDate()}</div>
            <h1 className="text-4xl font-bold tracking-tight">{greet()}, {user.name.split(" ")[0]}.</h1>
          ...
          {/* AI briefing */}
          <div className="bg-orange-50 dark:bg-orange-950/30 border border-orange-100 dark:border-orange-900 rounded-xl p-4 mb-8 flex items-start gap-3.5">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white flex-shrink-0">
              <Sparkles className="w-4 h-4" />
            </div>
            <div className="flex-1">
              <div className="text-[10px] font-bold text-orange-600 uppercase tracking-widest mb-1">Haxon AI · Daily briefing</div>
                ...
                <Tooltip.Root>
                  <Tooltip.Trigger asChild>
                    <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                      onClick={q.onClick}
                      className="flex items-center gap-3 p-3.5 w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-left hover:border-orange-300 dark:hover:border-orange-700 hover:shadow-sm transition-all group">
          ...
          <div className="grid grid-cols-[1fr_320px] gap-8">
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="font-bold text-lg tracking-tight">Recent</div>
              ...
              <div className="font-bold text-lg tracking-tight mb-4">This week</div>
