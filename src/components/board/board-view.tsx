      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-[720px] max-h-[85vh] flex flex-col z-50 overflow-hidden">
          <div className="flex items-center px-5 py-3.5 border-b border-gray-100 dark:border-gray-800">
            ...
            <IconBtn icon={<X className="w-3.5 h-3.5" />} tooltip="Close" onClick={onClose} />
          </div>
          <div className="flex-1 overflow-auto grid grid-cols-[1fr_200px]">
            <div className="p-6 border-r border-gray-100 dark:border-gray-800">
              <input
                ...
                onBlur={(e) => updateCard(boardId, colId, card.id, { title: e.target.value })}
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-950 flex flex-col gap-4">
              {[
                { label: "Assignee", content: assignee ? <div className="flex items-center gap-2 text-sm"><UserAvatar user={assignee} size={18} />{assignee.name}</div> : <span className="text-sm text-gray-400">Unassigned</span> },
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="flex gap-3 p-5 h-full min-h-0">
        {board.columns.map((col) => <KanbanColumn key={col.id} col={col} boardId={board.id} />)}
        <AnimatePresence>
          {addingCol && (
            <motion.div
              initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 300 }} exit={{ opacity: 0, width: 0 }}
        ...
        <thead>
          <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 sticky top-0 z-10">
            ...
            {["Name", "Status", "Assignee", "Priority", "Due Date", "Tags"].map((col) => (
              <th key={col} className="px-3 py-2.5 text-left font-semibold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">{col}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          <tr className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
            ...
            {cols.map((col) => (
              <th key={col} onClick={() => handleSort(col)}
                className="px-3 py-2.5 text-left font-semibold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap cursor-pointer hover:text-gray-700 dark:hover:text-gray-200 select-none">
...
export default function BoardView({ params }: { params: Promise<{ workspaceId: string; boardId: string }> }) {
  ...
  const { boards, workspaces, moveCard, boardViews, setBoardView } = useAppStore();
